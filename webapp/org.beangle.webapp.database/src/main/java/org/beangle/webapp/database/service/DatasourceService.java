...
public class DatasourceService extends BaseServiceImpl {
	...
	private Map<String, DataSource> datasources = CollectUtils.newHashMap();

	public void setDatasources(Map<String, DataSource> datasources) {
		this.datasources = datasources;
	}

	public DataSource getDatasources(String name) {
		DataSource datasource = datasources.get(name);
		if (null == datasource) {
			OqlBuilder<DatasourceBean> builder = OqlBuilder.from(DatasourceBean.class, "ds");
			builder.where("ds.name=:name", name);
			...
			if (!beans.isEmpty()) {
				...
				ds.setDriverClassName(bean.getDiverClassName());
