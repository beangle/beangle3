...
[@b.grid  items=groups var="group"  sortable="true" target="grouplist"]
	...
	[@b.row]
		...
		[@b.col property="name" width="20%" title="common.name"]<a href="${b.url('!info')}?groupId=${group.id}">${group.name}</a>[/@]
		[@b.col width="15%" property="owner.name" title="common.creator"/]
		[@b.col width="15%" property="updatedAt" title="common.updatedAt"]${group.updatedAt?string("yyyy-MM-dd")}[/@]
		[@b.col width="15%" property="category.name" title="适用身份" /]
		[@b.col width="10%" property="enabled" title="common.status"][@enableInfo group.enabled/][/@]
		[@b.col title="设置权限"][@b.a target="_blank" href="authority!edit?group.id=${group.id}"]<img style="border:0px" src="${base}/static/icons/beangle/16x16/actions/config.png"/>设置权限[/@][/@]
	[/@]
[/@]
[@b.foot/]