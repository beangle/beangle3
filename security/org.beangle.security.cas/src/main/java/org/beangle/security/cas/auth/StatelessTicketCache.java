 ...
 * <p>
 * When a service ticket or proxy ticket is validated against the CAS server, it
 * is unable to be used again. Most types of callers are stateful and are
 * associated with a given <code>HttpSession</code>. This allows the affirmative
 * CAS validation outcome to be stored in the <code>HttpSession</code>, meaning
 * the removal of the ticket from the CAS server is not an issue.
 * </p>
 * <P>
 ...
 * same ticket to the Beangle Security secured application on each occasion. As
 * no <code>HttpSession</code> is available for such callers, the affirmative
 * CAS validation outcome cannot be stored in this location.
 * </p>
 * <P>
 ...
 * in-memory cache stores the <code>CasAuthentication</code>, effectively
 ...
 * that the stateless caller are not required to unnecessarily acquire fresh CAS
...
public interface StatelessTicketCache {
	 ...
	 * <P>
	 * The {@link CasAuthentication#getCredentials()} method is used to retrieve
