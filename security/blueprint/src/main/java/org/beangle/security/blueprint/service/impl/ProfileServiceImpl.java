...
import org.beangle.security.blueprint.model.Permission;
...
public class ProfileServiceImpl extends BaseServiceImpl implements ProfileService {
  ...
  public Profile get(Long id) {
    return entityDao.get(UserProfile.class, id);
  }
  ...
  public Object getProperty(Profile profile, Dimension field) {
    String property = profile.getProperty(field);
    if (null == property) return null;
    if ("*".equals(property)) {
      return getDimensionValues(field);
    } else {
      return unmarshal(property, field);
    }
  }
