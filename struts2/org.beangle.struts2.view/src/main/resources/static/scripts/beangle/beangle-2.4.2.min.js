...
(function(h){beangle.history||(jQuery.struts2_jquery.require("/struts/js/plugins/jquery.form"+jQuery.struts2_jquery.minSuffix+".js",function(){h.beangle.history={init:function(){document.location.protocol==="file:"&&alert("The HTML5 History API (and thus History.js) do not work on files, please upload it to a server.");var f=h.History;jQuery(document).ready(function(){f.Adapter.bind(h,"statechange",function(){var g=f.getState();if(jQuery.type((g.data||{}).target)!="undefined"&&jQuery.type((g.data||
{}).html)!="undefined")jQuery(g.data.target).empty(),jQuery(g.data.target).html(g.data.html)})})},back:function(f){return History.back(f)},forward:function(f){return History.forward(f)},historyGo:function(f,g){var a=f.indexOf(" ");if(a>=0)var b=f.slice(a,f.length),f=f.slice(0,a);jQuery.ajax({url:f,type:"GET",dataType:"html",complete:function(a,d,e){e=a.responseText;a.isResolved()&&(a.done(function(a){e=a}),a=b?jQuery("<div>").append(e.replace(rscript,"")).find(b):e,History.pushState({html:a,target:"#"+
