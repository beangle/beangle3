 ...
 * See http://felix.apache.org/site/apache-felix-framework-launching-and-embedding.html <br/>
 * Servlet config params:
 * <p>
 * struts.osgi.clearBundleCache: Defaults to "true" delete installed bundles when the container
 * starts
 * </p>
 * <p>
 * struts.osgi.logLevel: Defaults to "1". Felix log level. 1 = error, 2 = warning, 3 = information,
 * and 4 = debug
 * </p>
 * <p>
...
public class FelixOsgiHost implements OsgiHost {
  ...
  protected void startFelix() {
    ...
    if (LOG.isDebugEnabled()) LOG.debug("Storing bundles at [#0]", storageDir);
    ...
    if ("true".equalsIgnoreCase(cleanBundleCache)) {
      if (LOG.isDebugEnabled()) LOG.debug("Clearing bundle cache");
    ...
    try {
      ...
      // felix.start();
      if (LOG.isTraceEnabled()) LOG.trace("Apache Felix is running");
    } catch (Exception ex) {
    ...
    servletContext.setAttribute(OSGI_BUNDLE_CONTEXT, framework.getBundleContext());
  }
  ...
  protected Map<String, String> getRunLevelDirs(String dir) {
    ...
    try {
      ...
      if (url != null) {
        ...
        } else if (LOG.isWarnEnabled()) LOG.warn("Unable to read [#0] directory", dir);
      } else if (LOG.isWarnEnabled()) LOG.warn("The [#0] directory was not found", dir);
    } catch (Exception e) {
      if (LOG.isWarnEnabled()) LOG.warn("Unable load bundles from the [#0] directory", e, dir);
    }
    return dirs;
  }
  ...
  protected void addSpringOSGiSupport() {
    // see the javadoc for
    // org.springframework.osgi.web.context.support.OsgiBundleXmlWebApplicationContext for more
    // details
    // OsgiBundleXmlWebApplicationContext expects the the BundleContext to be set in the
    // ServletContext under the attribute
    ...
    try {
      ...
      servletContext.setAttribute(key, framework.getBundleContext());
    ...
    for (String rootPackage : rootPackages) {
      try {
        ...
        for (Map.Entry<URL, Set<String>> entry : subpackagesMap.entrySet()) {
          ...
          if (StringUtils.isBlank(version)) version = getVersion(url);
        ...
        if (LOG.isErrorEnabled()) LOG.error("Unable to find subpackages of [#0]", e, rootPackage);
      }
    }
  ...
  /**
   * Gets the version used to export the packages. it tries to get it from MANIFEST.MF, or the file
   * name
   */
    ...
    if ("jar".equals(url.getProtocol())) {
      try {
        ...
        if (manifest != null) {
          ...
          if (StringUtils.isNotBlank(version)) { return getVersionFromString(version); }
        } else {
  ...
  protected Properties getProperties(String fileName) {
    ...
    try {
      ...
      if (LOG.isErrorEnabled()) LOG.error("Unable to read property file [#]", fileName);
  ...
  public Map<String, Bundle> getActiveBundles() {
    Map<String, Bundle> bundles = new HashMap<String, Bundle>();
    for (Bundle bundle : framework.getBundleContext().getBundles()) {
      if (bundle.getState() == Bundle.ACTIVE) bundles.put(bundle.getSymbolicName(), bundle);
    }
  ...
  public void destroy() throws Exception {
    framework.stop();
    if (LOG.isTraceEnabled()) LOG.trace("Apache Felix has stopped");
  }
