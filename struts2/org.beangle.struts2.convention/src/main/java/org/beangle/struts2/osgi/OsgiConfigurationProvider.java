...
public class OsgiConfigurationProvider implements PackageProvider, BundleListener {
  ...
  public void init(Configuration configuration) throws ConfigurationException {
    osgiHost = (OsgiHost) servletContext.getAttribute(ActionServlet.OSGI_HOST);
    ...
    // loading as well
    servletContext
        .setAttribute(ClassLoaderInterface.CLASS_LOADER_INTERFACE, new BundleClassLoaderInterface());
  }

  public synchronized void loadPackages() throws ConfigurationException {
    if (LOG.isTraceEnabled()) LOG.trace("Loading packages from XML and Convention on startup");
  ...
  protected void loadConfigFromBundle(Bundle bundle) {
    ...
    try {
      // the Convention plugin will use BundleClassLoaderInterface from the ActionContext to find
      // resources
      // and load classes
      ...
      for (PackageConfig pkg : loader.loadPackages(bundle, bundleContext, objectFactory, fileManager,
      ...
      // Convention
      // get the existing packages before reloading the provider (se we can figure out what are the
      // new packages)
      Set<String> packagesBeforeLoading = new HashSet(configuration.getPackageConfigNames());

      PackageProvider conventionPackageProvider = configuration.getContainer().getInstance(
          PackageProvider.class, "convention.packageProvider");
      if (conventionPackageProvider != null) {
        if (LOG.isTraceEnabled()) LOG.trace("Loading Convention config from bundle [#0]", bundleName);
      ...
      if (this.configuration.getRuntimeConfiguration() != null) {
        // if there is a runtime config, it meas that this method was called froma bundle start
        // event
  ...
  protected boolean shouldProcessBundle(Bundle bundle) {
    return null != bundle.getResource("struts-plugin.xml");
  ...
  public void destroy() {
    try {
      ...
      if (LOG.isErrorEnabled()) LOG.error("Failed to stop OSGi container", e);
    }
  }
  ...
  public void bundleChanged(BundleEvent bundleEvent) {
    ...
    if (bundleName != null && shouldProcessBundle(bundle)) {
      ...
      case BundleEvent.STARTED:
        if (LOG.isTraceEnabled())
          LOG.trace("The bundlde [#0] has been activated and will be scanned for struts configuration",
              bundleName);
        loadConfigFromBundle(bundle);
        break;
  ...
  protected void onBundleStopped(Bundle bundle) {
    ...
    if (!packages.isEmpty()) {
      if (LOG.isTraceEnabled())
        LOG.trace("The bundle [#0] has been stopped. The packages [#1] will be disabled",
