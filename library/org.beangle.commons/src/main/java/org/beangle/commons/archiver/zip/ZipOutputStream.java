...
public class ZipOutputStream extends FilterOutputStream {
	 ...
	 * https://issues.apache.org/bugzilla/show_bug.cgi?id=45396 Using a buffer
	 * size of 8 kB proved to be a good compromise
	 */
	 ...
	 * @since 1.1
	 */
	private final Map offsets = CollectUtils.newHashMap();
	...
	/**
	 * The zip encoding to use for filenames and the file comment. This field is
	 * of internal use and will be set in {@link #setEncoding(String)}.
	 */
	...
	public void closeEntry() throws IOException {
		...
		if (entry.getMethod() == DEFLATED) {
			...
			if (entry.getCrc() != realCrc) { throw new ZipException("bad CRC checksum for entry "
	...
	public void setLevel(int level) {
		if (level < Deflater.DEFAULT_COMPRESSION || level > Deflater.BEST_COMPRESSION) { throw new IllegalArgumentException(
	...
	protected void writeDataDescriptor(ZipEntry ze) throws IOException {
		if (ze.getMethod() != DEFLATED || raf != null) { return; }
		writeOut(DD_SIG);
		writeOut(ZipLong.getBytes(entry.getCrc()));
		writeOut(ZipLong.getBytes(entry.getCompressedSize()));
		writeOut(ZipLong.getBytes(entry.getSize()));
	...
	private void writeVersionNeededToExtractAndGeneralPurposeBits(final int zipMethod,
			final boolean utfFallback) throws IOException {
