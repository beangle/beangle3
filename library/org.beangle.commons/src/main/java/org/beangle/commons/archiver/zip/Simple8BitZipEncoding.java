 ...
 * <ul>
 * <li>Characters 0x0000 to 0x007f are encoded as the corresponding byte values 0x00 to 0x7f.</li>
 * <li>All byte codes from 0x80 to 0xff are mapped to a unique unicode character in the range 0x0080
 * to 0x7fff. (No support for UTF-16 surrogates)
 * </ul>
 * <p>
...
class Simple8BitZipEncoding implements ZipEncoding {
	...
	private static final class Simple8BitChar implements Comparable {
		...
		public String toString() {
			return "0x" + Integer.toHexString(0xffff & (int) unicode) + "->0x"
					+ Integer.toHexString(0xff & (int) code);
		}
	}
	...
	private Simple8BitChar encodeHighChar(char c) {
		...
		if (i0 >= this.reverseMapping.size()) { return null; }

		Simple8BitChar r = (Simple8BitChar) this.reverseMapping.get(i0);

		if (r.unicode != c) { return null; }
	...
	public boolean canEncode(String name) {
		...
		for (int i = 0; i < name.length(); ++i) {
			...
			if (!this.canEncodeChar(c)) { return false; }
		}
