...
public class ZipFile {
	...
	private Map populateFromCentralDirectory() throws IOException {
		Map noEFS = CollectUtils.newHashMap();
		...
		if (sig != cfhSig && startsWithLocalFileHeader()) { throw new IOException(
	...
	private void resolveLocalFileHeaderData(Map entriesWithoutEFS) throws IOException {
		...
		while (e.hasMoreElements()) {
			...
			while (lenToSkip > 0) {
				...
				if (skipped <= 0) { throw new RuntimeException("failed to skip file name in"
						+ " local file header"); }
				lenToSkip -= skipped;
			}
	...
	private boolean startsWithLocalFileHeader() throws IOException {
		...
		for (int i = 0; i < start.length; i++) {
			if (start[i] != ZipOutputStream.LFH_SIG[i]) { return false; }
		}
		return true;
	}
