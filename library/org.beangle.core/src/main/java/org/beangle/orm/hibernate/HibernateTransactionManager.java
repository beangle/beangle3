...
public class HibernateTransactionManager extends AbstractPlatformTransactionManager implements
	 ...
	 * SessionFactoryBean with a specified "dataSource", the DataSource will be auto-detected: You
	 * can still explictly specify the DataSource, but you don't need to in this case.
	 * <p>
	 ...
	 * the <code>getCurrentSession()</code> call fails.
	 * <p>
	 ...
	 * {@link BeangleSessionContext}), in which case it subtly differs from the Spring-managed
	 * Session mode: The pre-bound Session will <i>not</i> receive a <code>clear()</code> call (on
	 * rollback) or a <code>disconnect()</code> call (on transaction completion) in such a scenario;
	 * this is rather left up to a custom CurrentSessionContext implementation (if desired).
	 */
	 ...
	 * <p>
	 * Default is "false", performing an implicit flush as part of the actual commit step. Switch
	 * this to "true" in order to enforce an explicit early flush right <i>before</i> the actual
	 * commit step.
	 * <p>
	 ...
	 * SessionFactoryBean, or at the Session level,
	 * and HibernateTransactionManager. It's preferable to set it on SessionFactoryBean or
	 * HibernateTransactionManager to avoid repeated configuration and guarantee consistent behavior
	 * in transactions.
	 * 
	 ...
	 * overriding Hibernate's default SQLException translation (which is based on Hibernate's
	 * Dialect for a specific target database).
	 * 
		...
		if (sessionHolder != null) {
			if (logger.isDebugEnabled()) {
				logger.debug("Found thread-bound Session ["
						+ SessionUtils.toString(sessionHolder.getSession()) + "] for Hibernate transaction");
			}
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			if (txObject.getSessionHolder() == null
				...
				Session newSession = (entityInterceptor != null ? getSessionFactory().openSession(
						entityInterceptor) : getSessionFactory().openSession());
				if (logger.isDebugEnabled()) {
	...
	protected void doCleanupAfterCompletion(Object transaction) {
		...
		if (txObject.isNewSession()) {
			if (logger.isDebugEnabled()) {
				logger.debug("Closing Hibernate Session [" + SessionUtils.toString(session)
						+ "] after transaction");
			}
			SessionUtils.closeSession(session);
		} else {
	 ...
	 * Unfortunately, this requires casting to SessionImpl, as of Hibernate 3.1. If that cast
	 * doesn't work, we'll simply assume we're safe and return <code>true</code>.
	 * 
	...
	protected DataAccessException convertJdbcAccessException(JDBCException ex,
		...
		return translator.translate("Hibernate flushing: " + ex.getMessage(), ex.getSQL(),
				ex.getSQLException());
	}
