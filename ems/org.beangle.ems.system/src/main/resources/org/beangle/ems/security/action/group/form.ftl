...
[@b.tabs]
	[@b.tab label="用户组基本信息"]
		[@b.form action="!save" title="ui.groupInfo" theme="list"]
			...
			[@b.select label="上级组" name="parent.id" value=(userGroup.parent.id)! style="width:200px;"  items=parents?sort_by("code") option=r"${item.code} ${item.name}" empty="..."/]
		...
		[/@]
	[/@]
	[#if userGroup.children?size==0 && userGroup.id??]
	[@b.tab label="动态用户组属性" target="group_restriction"/]
