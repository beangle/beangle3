...
<div [#if tag.parameters['cssClass']??]class="${tag.parameters['cssClass']}"[#else]class="girdbar-pgbar"[/#if] [#if tag.parameters['cssStyle']??]style="${tag.parameters['style']}"[/#if]>
  ...
  <a href="#" onclick="page_${tag.id}.goPage(${tag.page.previousPageIndex})" >${b.text("page.previous")}</a>
  [/#if]
  ...
  <a href="#" onclick="page_${tag.id}.goPage(${tag.page.nextPageIndex})" >${b.text("page.next")}</a>
