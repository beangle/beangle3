...
public class DispatchAction extends DispatchActionSupport {
    ...
    protected ActionForward redirect(HttpServletRequest request, String method, String message) throws Exception {
        return redirect(request, new Action("", method), message);
    }

    protected ActionForward redirect(HttpServletRequest request, Action action, String message) throws Exception {
        if (Strings.isNotEmpty(message))
            saveErrors(request.getSession(), ForwardSupport.buildMessages(new String[] { message }));
    ...
    protected ActionForward redirect(HttpServletRequest request, Action action, String message, String prefixes[])
            throws Exception {
        StringBuffer buf;
        URLCodec urlCodec;
        if (Strings.isNotEmpty(message))
