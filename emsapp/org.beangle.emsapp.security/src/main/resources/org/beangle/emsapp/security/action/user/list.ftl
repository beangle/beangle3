...
[@b.grid items=users var="user"]
	...
	bar.addItem("${b.text("action.export")}",action.exportData("name:登录名,fullname:姓名,mail:电子邮件,groups:用户组,creator.fullname:创建者,effectOn:账户生效时间,invalidOn:账户失效时间,passwordExpiredAt:密码失效时间,createdAt:创建时间,updatedAt:修改时间,enabled:状态",null,"&fileName=用户信息"));
	...
	[@b.row]
		...
		[@b.col property="mail" title="common.email" width="23%"/]
		[@b.col property="effectOn" title="有效期" width="20%"]${user.effectOn?string("yyyy-MM-dd")}~${(user.invalidOn?string("yyyy-MM-dd"))!}[/@]
		[@b.col property="passwordExpiredOn" title="密码失效日期" width="10%"]${(user.passwordExpiredOn?string("yyyy-MM-dd"))!}[/@]
